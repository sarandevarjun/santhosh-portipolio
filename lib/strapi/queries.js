...
const STRAPI_BASE = process.env.STRAPI_URL || "https://tvk-backend-production.up.railway.app";
...
const SECTION_DEFAULTS = {
  ...
  join: {
    ...
    description:
      "உறுப்பினராகப் பதிவு செய்து, தோகைமலை கிழக்கு ஒன்றியத்தின் வளர்ச்சியில் பங்கேற்குங்கள்.",
...
async function fetchCollection(path, mapper, fallback) {
  ...
  try {
    const json = await strapiFetch(`${path}?sort=order:asc&pagination[pageSize]=100&populate=image`);
...
export async function getLeadershipMembers() {
  return fetchCollection(
    ...
    (item) => ({
      ...
      image: getImageUrl(item) || "/images/CM.png",
...
export async function getMediaBlocks() {
  return fetchCollection(
    ...
    (item) => ({
      ...
      image: getImageUrl(item) || item.imageUrl || "",
    }),
